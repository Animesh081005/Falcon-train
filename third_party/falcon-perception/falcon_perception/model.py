...
def squared_relu_gate(packed: T, hidden_dim: int) -> T:
    """Squared-ReLU gate for interleaved ``[g0,u0,g1,u1,...]`` values.

    The upstream Triton kernel is an inference optimization and does not
    register an autograd formula. Use ordinary PyTorch operations while
    gradients are enabled so fine-tuning updates ``w13`` and earlier layers.
    """
    if torch.is_grad_enabled() and packed.requires_grad:
        gate = packed[..., 0::2]
        up = packed[..., 1::2]
...
class FalconPerception(nn.Module):
    ...
    def forward(
        ...
        # Training-only optimization: compute vocabulary logits only at selected
        # hidden-state positions. Inference leaves this as None and is unchanged.
        logit_positions: T | None = None,
    ):
        B, S = tokens.size()
        ...
        logits_BSV = self.output(h_BSD if logit_positions is None else h_BSD[logit_positions])
